     ...
     if (not ( os.path.isfile(path_annual) and os.path.isfile(path_quarterly) )):  
       ...
       if(not os.path.isdir(path+row['SYMBOL'])):
         ...
         if(not (os.path.isfile(path_annual))):
           ...
           print "annual data saved"   #Saves the the website in html format
           
           element_A = browser.find_element_by_xpath('//*[@id="quote-leaf-comp"]/section/div[3]/table/tbody') 
           #locates the table which has Balance Sheet data
         ...
         if(not (os.path.isfile(path_quarterly))):
           ...
           print "quarterly data saved"
           element_Q = browser.find_element_by_xpath('//*[@id="quote-leaf-comp"]/section/div[3]/table/tbody') #locates the table which has Balance Sheet data
