...
with open('IO_error.csv') as csvfile:
    ...
    for row in reader:
      ...
      path_annual = path + row['SYMBOL']+'/'+row['SYMBOL']+'_CF_annual_data.csv'
      path_quarterly = path + row['SYMBOL']+'/'+row['SYMBOL']+'_CF_quarterly_data.csv' 
        
      ticker_name = row['SYMBOL'] + '.NS'
      print ticker_name
      if (not ( os.path.isfile(path_annual) and os.path.isfile(path_quarterly) )):
        #profile = webdriver.FirefoxProfile('/home/suraj/.mozilla/firefox/u8ryvk9c.default')  
        ...
        url = 'https://finance.yahoo.com/quote/' +ticker_name +'/cash-flow?p=' +ticker_name
        browser.get(url)
        data = []
        if(not os.path.isdir(path+row['SYMBOL'])):
          ...
          if(not (os.path.isfile(path_annual))):
            ...
            #element_cf = browser.find_element(By.XPATH, '//*[@id="main-0-Quote-Proxy"]/section/div[2]/section/div/section/div[2]/button[1]')
            #element_cf = browser.find_element(By.XPATH, '//*[@id="quote-leaf-comp"]/section/div[1]/div[1]/a[2]/div/span')
            #element_cf.click()
           
            #html_source_annual = browser.page_source
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_CF_annual')
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_CF_annual_table')
            with codecs.open(fullpath, "w", encoding="utf-8") as f:
            ...
            plaintext_A = plaintext_A.replace(' -', ',-')
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_CF_annual_data.csv')
            file_A = open(fullpath,"w")
            file_A.write(plaintext_A)
            file_A.close()
            print "annual data written to table"
          else:
            print row['SYMBOL']+'_CF_annual_data.csv'+' already exists!'  
          ...
          if(not (os.path.isfile(path_quarterly))):        
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_CF_quarterly')
            with codecs.open(fullpath, "w", encoding="utf-8") as f:
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_CF_quarterly_table')
            with codecs.open(fullpath, "w", encoding="utf-8") as f:
            ...
            plaintext_Q = plaintext_Q.replace(' -', ',-')
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_CF_quarterly_data.csv')
            file_Q = open(fullpath,"w")
            file_Q.write(plaintext_Q)
            file_Q.close()
            print "quarterly data written to table"
          else:
            print row['SYMBOL']+'_CF_quarterly_data.csv'+' already exists!'        
