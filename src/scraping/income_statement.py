...
with open('IO_error.csv') as csvfile:
   ...
   for row in reader:
     ...
     path_annual = path + row['SYMBOL']+'/'+row['SYMBOL']+'_IS_annual_data.csv'
     path_quarterly = path + row['SYMBOL']+'/'+row['SYMBOL']+'_IS_quarterly_data.csv'
     ticker_name = row['SYMBOL'] + '.NS'
     print ticker_name
     
     if(not os.path.isdir(path+row['SYMBOL'])):
       os.mkdir(path + row['SYMBOL'])      
     elif (not ( os.path.isfile(path_annual) and os.path.isfile(path_quarterly) )):
       firefox_profile = webdriver.FirefoxProfile()
       firefox_profile.set_preference('permissions.default.image', 2)
       firefox_profile.set_preference('dom.ipc.plugins.enabled.libflashplayer.so', 'false')
       browser = webdriver.Firefox(firefox_profile=firefox_profile)
       url = 'https://finance.yahoo.com/quote/' +ticker_name +'/financials?p=' +ticker_name
       browser.get(url)
       data = []
       ...
       if(not (os.path.isfile(path_annual))):
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_IS_annual')
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_IS_annual_table')
            with codecs.open(fullpath, "w", encoding="utf-8") as f:
            ...
            plaintext_A = plaintext_A.replace(' -', ',-')
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_IS_annual_data.csv')
            file_A = open(fullpath,"w")
            file_A.write(plaintext_A)
            file_A.close()
            print "annual data written to table"
       else:
            print row['SYMBOL']+'_IS_annual_data.csv'+' already exists!'
       ...
       if(not (os.path.isfile(path_quarterly))):
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_IS_quarterly')
            with codecs.open(fullpath, "w", encoding="utf-8") as f:
            ...
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_IS_quarterly_table')
            with codecs.open(fullpath, "w", encoding="utf-8") as f:
            ...
            plaintext_Q = plaintext_Q.replace(' -', ',-')
            fullpath=os.path.join(path, row['SYMBOL'], row['SYMBOL'] +'_IS_quarterly_data.csv')
            file_Q = open(fullpath,"w")
            file_Q.write(plaintext_Q)
            file_Q.close()
            print "quarterly data written to table"
       else:
            print row['SYMBOL']+'_IS_quarterly_data.csv'+' already exists!'  
